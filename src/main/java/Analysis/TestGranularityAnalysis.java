...
import Analyse.StatementModel;
import Analyse.StringUtil;
...
public class TestGranularityAnalysis {
    ...
    String ROOT_PATH = "/Users/sunweisong/Desktop/Construct MLCloneBench/Automated Test Case Extraction/picasso/src/main/java";//
    ...
    public TestGranularityAnalysis(String rootPath) {
...
//        String filePath = "/Users/sunweisong/Desktop/src/com.squareup.picasso3+BitmapHunterTest+cancelSingleRequest+().java";
...
//            CompilationUnit cu = tga.constructCompilationUnit(null, filePath);
//            List<ClassOrInterfaceDeclaration> myClassList = tga.reduceTestGranularity(cu, testTargetList, "4");
//            if (myClassList == null) {
...
//    }
     ...
     * @author sunweisong
     */
    public List<ClassOrInterfaceDeclaration> reduceTestGranularity(CompilationUnit cu, List<String> testTargetList
            , String junitVersion) {
        //获取类变量列表（包含可能的内部类的变量）
         ...
         */
        List<StatementModel> statementModelList = analyzeAllStatementsInTestMethod(testMethodDeclaration, junitVersion);
...
//                System.err.println(statementModel.toString());
    ...
    private List<StatementModel> analyzeAllStatementsInTestMethod(MethodDeclaration testMethodDeclaration
        ...
        for (int i = 0; i < statements.size(); i++) {
            ...
            String regex = "^(assert|Assert.assert|Assertions.assert" +
                    "|junit.framework.Assert.assert|org.junit.Assert.assert|org.junit.jupiter.api.Assertions.assert)" +
                    "[A,D,E,F,I,L,N,S,T][a-zA-Z]*\\(.*?\\);$";
            if (content.matches(regex)) {
                /**
                 * Example:
                 * JUnit 3.x: junit.framework.Assert.assertEquals()
                 * JUnit 4.x: org.junit.Assert.assertEquals()
                 * JUnit 5.x: org.junit.jupiter.api.Assertions.assertEquals()
                 */
            ...
            if (statementModel.isAssertStatement()) {
                analyzeSuspiciousTestTarget(statementModel, junitVersion);
            }
     ...
     * @author sunweisong
     */
    private void analyzeSuspiciousTestTarget(StatementModel statementModel, String junitVersion) {
        ...
        if (numOfArguments == 1) {
            ...
            String assertMethodName = mce.getName().asString();
            if (assertMethodName.contains("Assert.") || assertMethodName.contains("Assertions.")) {
                int assertClassIndex = assertMethodName.indexOf("Assert.");
                if (assertClassIndex == -1) {
                    assertClassIndex = assertMethodName.indexOf("Assertions.");
                    assertMethodName = assertMethodName.substring(assertClassIndex + 11);
                } else {
                    assertMethodName = assertMethodName.substring(assertClassIndex + 7);
                }
                /**
                 * Example:
                 * JUnit 3.x: junit.framework.Assert.assertEquals()
                 ...
                 * JUnit 5.x: org.junit.jupiter.api.Assertions.assertEquals()
                 */
                int firstPoint = assertMethodName.indexOf(".");
                assertMethodName = assertMethodName.substring(firstPoint + 1);
            }
//            System.out.println(assertMethodName);
            if ("3".equals(junitVersion)) {
                // JUnit 3.x
                if (numOfArguments == 2) {
                    actualExpression = arguments.get(1);
                }
                if (numOfArguments == 3) {
                    ...
                    if (firstArgument.isStringLiteralExpr()) {
                         ...
                         */
                        actualExpression = arguments.get(2);
                    } else {
            ...
            if ("4".equals(junitVersion)) {
                // JUnit 4.x
                if (numOfArguments == 2) {
                    if ("assertThat".equals(assertMethodName) || "assertThrows".equals(assertMethodName)) {
                        /**
                         * Example: assertThat(T actual, Matcher<? super T> matcher)
                         */
                        actualExpression = arguments.get(0);
                    } else {
                        actualExpression = arguments.get(1);
                ...
                if (numOfArguments == 3) {
                    if ("assertThat".equals(assertMethodName) || "assertThrows".equals(assertMethodName)) {
                        /**
                         * Example: assertThat(String reason, T actual, Matcher<? super T> matcher)
                         */
                        actualExpression = arguments.get(1);
                    } else {
                        Expression firstArgument = arguments.get(0);
            ...
            if ("5".equals(junitVersion)) {
                // JUnit 5.x
                if (numOfArguments == 2) {
                    if ("assertFalse".equals(assertMethodName) || "assertTrue".equals(assertMethodName)

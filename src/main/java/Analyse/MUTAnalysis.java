...
//                importDependencyString += s + ",";
        ...
        methodInfoTable.setImportDependencies(importDependencyString);
...
//                methodDependencyString += "000000picasso"+target + ",";
        ...
        if (ConstructorDependencySet.size() > 0) {
            for (ConstructorDeclaration cd : ConstructorDependencySet) {
                String target = "";
                String[] dependencyComment = cd.getJavadoc().get().toText().split(System.lineSeparator());
                for (int i = 0; i < dependencyComment.length; i++) {
                    if (dependencyComment[i].contains("packagename") || dependencyComment[i].contains("classname") || dependencyComment[i].contains("methodname") || dependencyComment[i].contains("parametertype")) {
                        target += dependencyComment[i].split(":")[1] + "+";
                    }
...
//                methodDependencyString += "000000picasso"+target + ",";
        ...
        if (ConstructorClassDependencySet.size() > 0) {
            for (ClassOrInterfaceDeclaration coid : ConstructorClassDependencySet) {
                for (ConstructorDeclaration cd : coid.getConstructors()) {
                    String target = "";
                    String[] dependencyComment = cd.getJavadoc().get().toText().split(System.lineSeparator());
                    for (int i = 0; i < dependencyComment.length; i++) {
                        if (dependencyComment[i].contains("packagename") || dependencyComment[i].contains("classname") || dependencyComment[i].contains("methodname") || dependencyComment[i].contains("parametertype")) {
...
//                    methodDependencyString += "000000picasso"+target + ",";
    ...
    public void writeMUTClass(String testOrProduct, String packageName, String methodName, String typeString, List<String> writeFileImportList, List<String> externalMethodList, ClassOrInterfaceDeclaration myClass) {
        try {
            ...
            String outputFileName0 = "MUT Class/" + testOrProduct + "+" + packageName + "+" + filename + "+" + methodName + "+" + typeString + ".txt";
